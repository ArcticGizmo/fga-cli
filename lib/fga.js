...
class FGAClient {
  ...
  async setStoreFromName(name) {
    const resp = await this.getStoreByName(name);
    if (!resp) {
      throw new Error("Unable to find store for name");
    }

    this.setStore(resp.id);
  }
  ...
  async listModels() {
    return await this._getAllModels();
  }

  async _getAllModels(acc = []) {
    const resp = await this.getPagedModels();
    acc = acc.concat(resp.authorization_models);
    if (resp.continuation_token) {
      return await this._getAllModels(acc);
    }

    return acc;
  }

  async getPagedModels(token) {
